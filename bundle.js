(function (d3) {
    ...
    d3__namespace.json("hierarchy_2024-11-14-14-57-34.json").then(function (data) {

        const svg = d3__namespace.select("body").append("svg").attr('width', window.innerHeight).attr('height', window.innerHeight);

        svg.node().appendChild(SunburstSimple(data, 0, 1000 / 2));
    ...
    function SunburstSimple(data, cutoff, radius) {
        ...
        const innerR = d => ((radius * 2) / 10) * (d.dirLevel + 1) - d.y0*3;
        const outerR = d => ((radius * 2) / 10) * (d.dirLevel + 2) - d.y0*3;
        ...
        svg.append("g")
            ...
            .selectAll("path")
            .data(root.descendants().filter(d => d.dirLevel > cutoff))
            .join("path")
            .attr("fill", d => {
        ...
        svg.append("g")
            ...
            .selectAll("text")
            .data(root.descendants().filter(d => d.dirLevel > cutoff && ((outerR(d) - innerR(d)) * (d.x1 - d.x0)) > 5))
            .join("text")
