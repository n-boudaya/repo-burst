...
Promise.all([
    ...
    function changeTimestepMain(event) {
        ...
        d3.select(mainUI.get("timeStepSlider")).property("value", currentTimeStepMain);
        d3.select(mainUI.get("timeStepField")).property("value", currentTimeStepMain);
        d3.select(mainUI.get("timeStepName")).html(files[0][currentTimeStepMain-1].hierarchy);

        const start = d3.select(mainUI.get("startLevelSlider")).property("value");
        const stop = d3.select(mainUI.get("stopLevelSlider")).property("value");
    ...
    function changeTimestepSecond(event) {
        ...
        d3.select(secondaryUI.get("timeStepSlider")).property("value", currentTimeStepSecond);
        d3.select(secondaryUI.get("timeStepField")).property("value", currentTimeStepSecond);
        d3.select(secondaryUI.get("timeStepName")).html(files[0][currentTimeStepSecond-1].hierarchy);

        const start = d3.select(secondaryUI.get("startLevelSlider")).property("value");
        const stop = d3.select(secondaryUI.get("stopLevelSlider")).property("value");
    ...
    function setup(){
        ...
        d3.select(mainUI.get("timeStepSlider")).on("change", changeTimestepMain);
        d3.select(mainUI.get("timeStepField")).on("change", changeTimestepMain);
        d3.select(mainUI.get("timeStepSlider")).property("max", files[0].length);
        d3.select(mainUI.get("timeStepField")).property("max", files[0].length);
        d3.select(mainUI.get("maxStepValue")).html(files[0].length);
        d3.select(mainUI.get("timeStepSlider")).property("value", 1);
        d3.select(mainUI.get("timeStepField")).property("value", 1);
        d3.select(mainUI.get("timeStepName")).html(files[0][0].hierarchy);

        d3.select(secondaryUI.get("timeStepSlider")).on("change", changeTimestepSecond);
        d3.select(secondaryUI.get("timeStepField")).on("change", changeTimestepSecond);
        d3.select(secondaryUI.get("timeStepSlider")).property("max", files[0].length);
        d3.select(secondaryUI.get("timeStepField")).property("max", files[0].length);
        d3.select(secondaryUI.get("maxStepValue")).html(files[0].length);
        d3.select(secondaryUI.get("timeStepSlider")).property("value", 1);
        d3.select(secondaryUI.get("timeStepField")).property("value", 1);
        d3.select(secondaryUI.get("timeStepName")).html(files[0][0].hierarchy);
    }
    setup();
}).catch(function (err) {
    console.log(err);
...
d3.select("body").append("button").html("testSelect").on("click", selectAllArcs);

function selectAllArcs(){
    const mainSelection = d3.select("#mainGraph").selectAll("#shortArcs").selectAll("path");
    const secondarySelection = d3.select("#secondaryGraph").selectAll("#shortArcs").selectAll("path");
    ...
    const difference = d3.difference(mainArr.map(e=>e.getAttribute('path')), secondaryArr.map(e=>e.getAttribute('path')));
