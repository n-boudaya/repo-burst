...
Promise.all([
    ...
    d3.select("#timeStepSlider").on("change", changeTimestep);
    d3.select("#timeStepInput").on("change", changeTimestep);
    d3.select("#timeStepSlider").property("max", files[0].length);
    d3.select("#timeStepInput").property("max", files[0].length);
    d3.select("#timeStepName").html(files[0][0].hierarchy);
    ...
    function changeTimestep(event) {
        ...
        d3.select("#timeStepSlider").property("value", currentTimeStep);
        d3.select("#timeStepInput").property("value", currentTimeStep);
        d3.select("#timeStepName").html(files[0][currentTimeStep-1].hierarchy);
...
function refreshChart(hierarchy, dependency) {
    Promise.all([
        ...
        sunburstSVG.node().replaceChildren(zoomableSunburst(files[0], files[1], 3));
...
// The x coordinates contain the starting and end angles for elements in the partition, the y coordinates contain the hierarchy levels of the related file.
    ...
    function adjustStart(event) {
        ...
        d3.select("#startLvlCurrLvl").html(event.target.value);
